   	...
   	if ($('#about-page').is(":visible")) {
			...
			$(window).load(function(){
				...
				function filter_projects(tag)
				{
				  ...
				  if (tag!='*')
					window.location.hash+=tag.replace(substr(5),'aboutall');
				  if (tag=='*')
				  	window.location.hash+='#aboutall';
				}
				...
				if ($container.length) {
					...
					if (window.location.hash!='#aboutall')
					{
						filter_projects( '.' + window.location.hash.replace('#aboutall','') );
					}
				}
			})
		} else {
